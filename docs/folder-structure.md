...
`storage_root` (catalog-level) and `schema_storage_root`/`additional_schemas.*.storage_root` (schema-level) are all optional (IT-66), but **a brand-new catalog needs one of the two catalog-level options**: `CREATE CATALOG` fails live with "Metastore storage root URL does not exist ... please provide a storage location for the catalog" if the catalog has no `storage_root` and the metastore has none either — schema-level storage_root alone isn't sufficient to satisfy catalog creation, even though it's needed anyway for per-schema paths. In practice: set the catalog's `storage_root` to a registered External Location's base URL, then optionally set each schema's `storage_root` to a subpath nested under it for per-medallion-layer storage governance — see `environments/dev/main.tf`'s `databricks_external_location.vicmo` and `module.databricks_catalog_vicmo` for the pattern.
